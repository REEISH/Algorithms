...
int SelectTemplate(int *A, int start, int end)
{
    ...
    {
        int startArr = 5 * (i) + start;
        int endArr = 5 * (i) + start + 4;
        if (endArr > end)
        {
            endArr = end;
        }
        insertionSort(A, startArr, endArr);
        B[i] = A[(startArr + endArr) / 2 - 1];
    }
...
int SelectinLinearTime(int *A, int start, int end, int Order)
{
    ...
    {
        return SelectinLinearTime(A, start, parter - 1, Order);
    }
    else
    {
        return SelectinLinearTime(A, parter + 1, end, Order - NUM);
    }
}
