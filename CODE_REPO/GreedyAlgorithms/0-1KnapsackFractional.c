...
double KnapSackFractional(LONG *Weights, LONG *Cost, double *Result, int N, int W)
{
    int Item[N];
    ...
    for (int i = 0; i < N; i++)
    {
        if (Result[i] > 0)
        {
            CostVal += (Result[i] * Cost[i]) / (double)Weights[Item[i] - 1];
        }
    }
    return CostVal;
}
...
int main()
{
    ...
    scanf("%d", &W);
    double Result[N];
    double result = KnapSackFractional(Weights, Cost, Result, N, W);
    printArray(Result, N);
    printf("The maximum value we got is :%f", result);
